...
⊢supp (⊢𝛌{B = B}{b}{x} q₀ (q₁ ∉∪ q₁') h₀ h₁) = ⊆ub
  ...
  (⊆ub
    (⊢supp h₀ ∘ ∈∪₁)
    (⊆ub (⊢supp¹ B x h₁ q₁) λ()))
⊢supp (⊢∙{B = B}{a}{x = x} q₀ q₁ h₀ h₁ h₂) = ⊆ub
  (⊆ub (⊢supp q₀ ∘ ∈∪₁) (⊆ub (⊢supp q₁ ∘ ∈∪₁) λ()))
  ((⊆ub (⊢supp¹ B x h₁ h₂) (⊢supp q₁ ∘ ∈∪₁)) ∘ supp[] B a)
⊢supp (⊢𝐈𝐝 q₀ q₁ h) = ⊆ub
...
⊢supp (⊢𝐉{C = C}{b = b}{e = e}{x = x}{y}
  q₀ q₁ q₂ q₃ q₄ (##:: q₅ (##:: (_ ∉∪ q₅') ##◇)) h₀ h₁) = ⊆ub
  (⊆ub
    (⊢supp² C x y q₀ q₅' q₅)
    (⊆ub
      (⊢supp q₁ ∘ ∈∪₁)
...
⊢supp (⊢𝐧𝐫𝐞𝐜 {C = C} {a = a} {c₊ = c₊} {x} {y}
  q₀ q₁ q₂ (##:: (q₃ ∉∪ q₃') (##:: (_ ∉∪ q₃'' ∉∪ q₃''') ##◇)) h) = ⊆ub
  (⊆ub
    (⊢supp¹ C x h q₃'')
    (⊆ub
      (⊢supp q₀ ∘ ∈∪₁)
...
⊢supp (𝛌Cong{B = B}{b}{b'}{x} q₀ (q₁ ∉∪ q₁' ∉∪ q₁'') h₀ h₁) = ⊆ub
  ...
  (⊆ub
    ...
    (⊆ub
      (⊢supp h₀ ∘ ∈∪₁)
      (⊆ub (⊢supp¹ B x h₁ q₁) λ())))
⊢supp (∙Cong{B = B}{a = a}{x = x} q₀ q₁ h₀ h₁ h₂) = ⊆ub
  (⊆ub
    (⊢supp q₀ ∘ ∈∪₁)
    (⊆ub (⊢supp q₁ ∘ ∈∪₁) λ()))
  (⊆ub
...
⊢supp (𝐍𝐚𝐭Beta₊{C = C}{c₀}{a}{c₊}{x}{y}
  ...
  (⊆ub
    ...
    ((⊆ub
      ...
      (⊆ub (⊢supp q₂ ∘ ∈∪₁) λ()))
      ∘ supp[] C (𝐬𝐮𝐜𝐜 a)))
⊢supp{Γ} (𝚷Eta{A = A}{B}{b}{x} q₀ q₁ h) = ⊆ub
  (⊢supp q₀ ∘ ∈∪₁)
  (⊆ub
    (⊆ub
      (⊆ub
        (⊢supp q₀ ∘ ∈∪₁ ∘ suppAbs x b)
        (⊆ub (∉⊆ (#abs x (𝐯 x)) (∈∪₂ ∘ suppAbs x (𝐯 x))) λ()))
      λ())
    (⊆ub
      (⊢supp h ∘ ∈∪₁)
      (⊢supp q₀ ∘ ∈∪₂ ∘ ∈∪₂)))
