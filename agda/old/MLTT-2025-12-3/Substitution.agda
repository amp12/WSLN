...
liftSb{σ = σ}{Γ}{Γ'}{A}{x}{x'} ⊢σ ⊢A ⊢σA = ∷ (wkSb x' ⊢σA ⊢σ') ⊢A ⊢A'
  where
  ⊢σ' : Γ' ⊢ˢ (x := 𝐯 x')σ ∶ Γ
...
sbJg{σ}{Γ' = Γ'} p (⊢𝚷{ℓ' = ℓ'}{A}{B}{x} q₀ q₁ q₂) =
  ⊢𝚷{x = x'}
    (sbJg p q₀)
    (subst (λ B' → (Γ' ⸴ x' ∶ σ * A) ⊢ B' ∶ 𝐒𝐞𝐭 ℓ')
...
sbJg{σ}{Γ' = Γ'} p (⊢𝛌{ℓ' = ℓ'}{A}{B}{b}{x} q₀ (q₁ ∉∪ q₁') h₀ h₁) =
  ⊢𝛌{x = x'}
...
sbJg{σ}{Γ' = Γ'} p (⊢∙{ℓ' = ℓ'}{A}{B}{a}{b}{x} q₀ q₁ q₂ q₃ h)
  rewrite sb⟨⟩ σ B a =
...
sbJg{σ}{Γ' = Γ'} p (⊢𝐉{ℓ}{ℓ'}{A}{C}{a}{b}{c}{e}{x}{y}
  q₀ q₁ q₂ q₃ q₄ q₅ q₆ h₀ h₁)
  rewrite sb⟨⟩² σ C b e =
  ...
  h₁' : (Γ' ⸴ x' ∶ σ * A) ⊢ 𝐈𝐝(σ * A)(σ * a)(𝐯 x') ∶ 𝐒𝐞𝐭 ℓ
  h₁' = subst (λ I → (Γ' ⸴ x' ∶ σ * A) ⊢ I ∶ 𝐒𝐞𝐭 ℓ)
    eq
    (sbJg (liftSb p h₀ (sbJg p h₀)) h₁)
  ...
  q₀' : (Γ' ⸴ x' ∶ σ * A ⸴ y' ∶ 𝐈𝐝(σ * A)(σ * a)(𝐯 x')) ⊢
    (σ * C) ⟨ x' ⸴ y' ⟩ ∶ 𝐒𝐞𝐭 ℓ'
  q₀' = subst (λ C →
    (Γ' ⸴ x' ∶ σ * A ⸴ y' ∶ 𝐈𝐝(σ * A)(σ * a)(𝐯 x')) ⊢ C ∶ 𝐒𝐞𝐭 ℓ')
    eq'
...
sbJg{σ}{Γ' = Γ'} p (⊢𝐧𝐫𝐞𝐜{ℓ}{C}{c₀}{a}{c₊}{x}{y} q₀ q₁ q₂ (q₃ ∉∪ q₃') q₄ h)
  rewrite sb⟨⟩ σ C a =
  -- helper hypotheses used
  ⊢𝐧𝐫𝐞𝐜{x = x'}{y'}
    q₀'
...
sbJg{σ}{Γ' = Γ'} p (𝚷Cong{ℓ' = ℓ'}{A}{B = B}{B'}{x}
  q₀ q₁ (q₂ ∉∪ q₂') h) =
  𝚷Cong{x = x'}
    (sbJg p q₀)
    (subst₂ (λ C C' → (Γ' ⸴ x' ∶ σ * A) ⊢ C ＝ C' ∶ 𝐒𝐞𝐭 ℓ')
      (sbUpdate⟨⟩ σ x (𝐯 x') B q₂)
      (sbUpdate⟨⟩ σ x (𝐯 x') B' q₂')
      (sbJg (liftSb p h (sbJg p h)) q₁))
  x'#
...
sbJg{σ}{Γ' = Γ'} p (𝛌Cong{ℓ' = ℓ'}{A}{B = B}{b}{b'}{x}
  q₀ q₁ (q₂ ∉∪ q₂' ∉∪ q₂'') h₀ h₁) =
  𝛌Cong{x = x'}
    (sbJg p q₀)
...
sbJg{σ}{Γ' = Γ'} p (∙Cong{ℓ' = ℓ'}{A}{A'}{B}{B'}{a}{a'}{b}{b'}{x}
  q₀ q₁ q₂ q₃ (q₄ ∉∪ q₄') h₀ h₁)
  rewrite sb⟨⟩ σ B a =
  -- helper hypotheses used
  ∙Cong{x = x'}
...
sbJg{σ}{Γ' = Γ'} p (𝐉Cong{ℓ}{ℓ'}{A}{C}{C'}{a}{a'}{b}{b'}{c}{c'}{e}{e'}{x}{y}
  ...
  q₃' : Γ' ⊢ σ * c ＝ σ * c' ∶ (σ * C) ⟨ σ * a ⸴ 𝐫𝐞𝐟𝐥 (σ * a) ⟩
  q₃' = subst (λ C' → Γ' ⊢ σ * c ＝ σ * c' ∶ C')
    (sb⟨⟩² σ C a (𝐫𝐞𝐟𝐥 a))
    (sbJg p q₃)
  ...
  q₀' : (Γ' ⸴ x' ∶ σ * A ⸴ y' ∶ 𝐈𝐝(σ * A)(σ * a)(𝐯 x')) ⊢
    (σ * C) ⟨ x' ⸴ y' ⟩ ＝ (σ * C') ⟨ x' ⸴ y' ⟩ ∶ 𝐒𝐞𝐭 ℓ'
  q₀' = subst₂ (λ D D' →
    (Γ' ⸴ x' ∶ σ * A ⸴ y' ∶ 𝐈𝐝(σ * A)(σ * a)(𝐯 x')) ⊢ D ＝ D' ∶ 𝐒𝐞𝐭 ℓ')
    eq'
    eq''
    (sbJg (liftSb² p h₀ h₁ refl eq (sbJg p h₀) h₁') q₀)
...
sbJg{σ}{Γ' = Γ'} p (𝐧𝐫𝐞𝐜Cong{ℓ}{C}{C'}{c₀}{c₀'}{a}{a'}{c₊}{c₊'}{x}{y}
...
sbJg{σ}{Γ' = Γ'} p (𝚷Beta{ℓ' = ℓ'}{A}{a}{B}{b}{x} q₀ q₁ (q₂ ∉∪ q₂') h₀ h₁)
  rewrite sb⟨⟩ σ b a
  | sb⟨⟩ σ B a =
  -- helper hypotheses used
  𝚷Beta{x = x'}
...
sbJg{σ}{Γ' = Γ'} p (𝐈𝐝Beta{ℓ}{ℓ'}{A}{C}{a}{c}{x}{y}
  q₀ q₁ q₂ q₃ q₄ h₀ h₁)
  ...
  q₀' : (Γ' ⸴ x' ∶ σ * A ⸴ y' ∶ 𝐈𝐝(σ * A)(σ * a)(𝐯 x')) ⊢
    (σ * C) ⟨ x' ⸴ y' ⟩ ∶ 𝐒𝐞𝐭 ℓ'
  q₀' = subst (λ D →
    (Γ' ⸴ x' ∶ σ * A ⸴ y' ∶ 𝐈𝐝(σ * A)(σ * a)(𝐯 x')) ⊢ D ∶ 𝐒𝐞𝐭 ℓ')
    eq'
    (sbJg (liftSb² p h₀ h₁ refl eq (sbJg p h₀) h₁') q₀)
...
sbJg{σ}{Γ' = Γ'} p (𝐍𝐚𝐭Beta₀{ℓ}{C}{c₀}{c₊}{x}{y} q₀ q₁ (q₂ ∉∪ q₂') q₃ h)
  rewrite sb⟨⟩ σ C 𝐳𝐞𝐫𝐨 =
  -- helper hypotheses used
...
sbJg{σ}{Γ' = Γ'} p (𝐍𝐚𝐭Beta₊{ℓ}{C}{c₀}{a}{c₊}{x}{y} q₀ q₁ q₂ (q₃ ∉∪ q₃') q₄ h)
...
sbJg{σ}{Γ' = Γ'} p (𝚷Eta{ℓ}{ℓ'}{A}{B}{b}{x} q₀ q₁ h) =
  -- helper hypothesis used
...
ssbUpdate{Γ = Γ}{A}{a} q h = sbUpdate
  (⊢ι (⊢ok q))
  h
...
ssb＝Update{Γ = Γ}{A}{a}{a'} q h = sb＝Update
  (sb＝Refl (⊢ι (⊢ok q)))
  h
...
lift＝Sb{σ = σ}{σ'}{Γ}{Γ'}{A}{x}{x'} p q h =
  ...
  q'' rewrite updateFresh σ x (𝐯 x') A (∉∪₁ (⊢# q it))
      | :=Eq{f = σ}{𝐯 x'} x
      | :=Eq{f = σ'}{𝐯 x'} x =
    Refl (⊢𝐯 (∷ (sbJg h q) (okSb＝ p)) (isInNew refl))
...
＝sbTm{σ = σ}{σ'}{Γ' = Γ'} p (⊢𝚷{ℓ' = ℓ'}{A}{B}{x} q₀ q₁ q₂) h =
  -- helper hypotheses used
  𝚷Cong{x = x'}
    (＝sbTm p q₀ h)
    (subst₂ (λ C C' → (Γ' ⸴ x' ∶ σ * A) ⊢ C ＝ C' ∶ 𝐒𝐞𝐭 ℓ')
...
＝sbTm{σ = σ}{σ'}{Γ' = Γ'} p (⊢𝛌{ℓ' = ℓ'}{A}{B}{b}{x}
  q₀ (q₁ ∉∪ q₁') h₀ h₁) h =
  -- helper hypotheses used
  𝛌Cong{x = x'}
    (＝sbTm p h₀ h)
...
＝sbTm{σ = σ}{σ'}{Γ' = Γ'} p (⊢𝐉{ℓ}{ℓ'}{A}{C}{a}{b}{c}{e}{x}{y}
  q₀ q₁ q₂ q₃ q₄ q₅ q₆ h₀ h₁) h
  rewrite sb⟨⟩² σ C b e =
  -- helper hypotheses used
  𝐉Cong{x = x'}{y'}
...
＝sbTm{σ = σ}{σ'}{Γ' = Γ'} p (⊢𝐧𝐫𝐞𝐜{ℓ}{C}{c₀}{a}{c₊}{x}{y}
  q₀ q₁ q₂ (q₃ ∉∪ q₃') q₄ q₅) h
  rewrite sb⟨⟩ σ C a =
  -- helper hypotheses used
  𝐧𝐫𝐞𝐜Cong {x = x'} {y'}
    (subst₂ (λ D D' → (Γ' ⸴ x' ∶ 𝐍𝐚𝐭) ⊢ D ＝ D' ∶ 𝐒𝐞𝐭 ℓ)
      (eq σ)
      (eq σ')
