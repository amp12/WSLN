...
Oksupp (∷ p _)  (isInNew refl) = ∈∪₁ ∘ ⊢supp p ∘ ∈∪₁
...
⊢supp (⊢𝚷{B = B}{x} q₀ q₁ h) =
  -- helper used here
  ⊆ub (⊆ub (⊢supp q₀ ∘ ∈∪₁) (⊆ub (⊢supp¹ B x q₁ h) λ())) λ()
⊢supp (⊢𝛌{B = B}{b}{x} q₀ (q₁ ∉∪ q₁') h₀ h₁) =
  -- helper used here
  ⊆ub
...
⊢supp (⊢𝐈𝐝 q₀ q₁ q₂) = ⊆ub (⊆ub (⊢supp q₂ ∘ ∈∪₂)
  (⊆ub (⊢supp q₁ ∘ ∈∪₁) (⊆ub (⊢supp q₂ ∘ ∈∪₁) λ()))) λ()
⊢supp (⊢𝐫𝐞𝐟𝐥 q _) = ⊆ub
  (⊆ub (⊢supp q ∘ ∈∪₁) λ())
  (⊆ub (⊢supp q ∘ ∈∪₂)
