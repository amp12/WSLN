...
rnJg p (Ty→Tm q) = Ty→Tm (rnJg p q)

rnJg{ρ}{Γ' = Γ'} p (⊢𝛌{l}{l'}{A = A}{B}{b}{x}
  q₀ (q₁ ∉∪ q₁') h₀ h₁) =
  ⊢𝛌{x = x'}
...
rnJg p (TyTrans q₀ q₁) = TyTrans (rnJg p q₀) (rnJg p q₁)
...
rnJg p (TmTrans q q₁) = TmTrans (rnJg p q) (rnJg p q₁)

rnJg p (＝conv q₀ q₁) = ＝conv (rnJg p q₀) (rnJg p q₁)

rnJg p (=Ty→Tm q) = =Ty→Tm (rnJg p q)

rnJg p (⊢Reflect q₀ q₁ q₂ q₃) = ⊢Reflect
  (rnJg p q₀)
  (rnJg p q₁)
  (rnJg p q₂)
  (rnJg p q₃)

rnJg{ρ}{Γ' = Γ'} p (𝛌Cong{l}{l'}{A = A}{B}{b}{b'}{x}
  q₀ (q₁ ∉∪ q₁' ∉∪ q₁'') h₀ h₁) =
  𝛌Cong{x = x'}
...
rnJg{ρ}{Γ' = Γ'} p (∙Cong{l}{l'}{A}{B}{a}{a'}{b}{b'}{x}
  q₀ q₁ q₂ h₀ h₁)
  rewrite rn⟨⟩ ρ B a =
  -- helper hypotheses used
  ∙Cong{x = x'}
...
rnJg{ρ}{Γ' = Γ'} p (𝚷Eta{l}{l'}{A = A}{B}{b}{x} q₀ q₁ h) =
  -- helper hypothesis used
  ...
  eq : 𝛌 (x' ． (ρ * b) ∙ 𝐯 x') ≡
    ρ * 𝛌 (x ． b ∙ 𝐯 x)
  eq rewrite rnAbs ρ x x' (b ∙ 𝐯 x) f
     | updateFreshRn ρ x x' b x#b
     | :=Eq{f = ρ}{x'} x = refl

rnJg p (UIP q q₁ q₂ q₃) = UIP
  (rnJg p q)
  (rnJg p q₁)
  (rnJg p q₂)
  (rnJg p q₃)
