...
⊢ι (∷{l}{Γ}{A}{x} q q') = ∷
  (wkSb _ q (⊢ι q'))
  q
  (⊢𝐯 (∷⁻ q) (subst (λ A' → (x , A' , l) isIn Γ ⸴ x ∶ A ⦂ l)
    (symm (sbUnit A))
    isInNew))
...
sbJg p (TyTrans q₀ q₁) = TyTrans (sbJg p q₀) (sbJg p q₁)
...
sbJg p (⊢Reflect q₀ q₁ q₂ q₃) = ⊢Reflect
  (sbJg p q₀)
  (sbJg p q₁)
  (sbJg p q₂)
  (sbJg p q₃)
...
sbJg{σ}{Γ' = Γ'} p (∙Cong{l}{l'}{A}{B}{a}{a'}{b}{b'}{x}
  q₀ q₁ q₂ h₀ h₁)
  rewrite sb⟨⟩ σ B a =
  -- helper hypotheses used
  ∙Cong{x = x'}
...
sbJg{σ}{Γ' = Γ'} p (𝚷Eta{l}{l'}{A}{B}{b}{x} q₀ q₁ h) =
  ...
  f :
    ...
    → ----------------------
    x' # σ y
  f y (∈∪₁ y∈b) ¬xy = ⊆∉ (sbDom p (⊢supp q₀ (∈∪₁ y∈b))) x'#Γ'
  f y (∈∪₂ (∈∪₁ ∈｛｝)) ¬xy = Øelim (¬xy refl)

  eq : 𝛌(x' ． (σ * b) ∙ 𝐯 x') ≡
    σ * 𝛌(x ． b ∙ 𝐯 x)
  eq rewrite sbAbs σ x x' (b ∙ 𝐯 x) f
     | updateFresh σ x (𝐯 x') b x#b
     | :=Eq{f = σ}{𝐯 x'} x = refl

sbJg p (UIP q₀ q₁ q₂ q₃) = UIP
  (sbJg p q₀)
  (sbJg p q₁)
  (sbJg p q₂)
  (sbJg p q₃)
...
sbUpdate{l}{Γ' = Γ'}{σ}{A}{a}{x} ⊢σ ⊢a ⊢A = ∷
  ...
  (subst₂ (λ z Z → Γ' ⊢ z ∶ Z ⦂ l)
    (symm (:=Eq{f = σ}{a} x))
    (symm (updateFresh σ x a A (⊢# ⊢A it)))  -- (∉∪₁ (⊢# ⊢A it))))
    ⊢a)
...
＝sbTy _ (⊢𝐔 _) h = TyRefl (⊢𝐔 (okSb h))

＝sbTy{σ = σ}{σ'}{Γ' = Γ'} p (⊢𝚷{l}{l'}{A = A}{B}{x} q₀ q₁ q₂) h =
  -- helper hypotheses used
  𝚷Cong{x = x'}
    (＝sbTy p q₀ h)
    (subst₂ (λ C C' → (Γ' ⸴ x' ∶ σ * A ⦂ l) ⊢ C ＝ C' ⦂ l')
      (sbUpdate⟨⟩ σ x (𝐯 x') B q₂)
      (sbUpdate⟨⟩ σ' x (𝐯 x') B q₂)
      (＝sbTy (lift＝Sb p q₀ h) q₁ (liftSb⁻ h q₀)))
    x'#
    (sbJg h q₀)
  where
  S = supp ((σ * B , σ' * B) , Γ')
...
＝sbTy p (⊢𝐈𝐝 q₀ q₁ h) h' = 𝐈𝐝Cong
  (＝sbTm p q₀ h')
  (＝sbTm p q₁ h')
  (sbJg h' h)

＝sbTy p (⊢𝐍𝐚𝐭 _) _ = TyRefl (⊢𝐍𝐚𝐭 (okSb＝ p))

＝sbTy p (Tm→Ty q) h = =Tm→Ty (＝sbTm p q h)
...
＝sbTm p (Ty→Tm q) h = =Ty→Tm (＝sbTy p q h)
...
＝sbTm p (⊢𝐫𝐞𝐟𝐥 q₀ q₁) h = TmRefl (⊢𝐫𝐞𝐟𝐥 (sbJg h q₀) (sbJg h q₁))
